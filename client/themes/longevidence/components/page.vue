      ...
      v-container.grey.pa-0(fluid, :class='$vuetify.theme.dark ? `darken-4-l3` : `lighten-4`')
        ...
        v-row.page-header-section.page-header-section--fixed-height(v-else, no-gutters, align-content='center')
          v-col.page-col-content.is-page-header.page-header-col-auto-margin(
            :offset-xl='hasDesktopLeftTocColumn ? 2 : 0'
            :offset-lg='hasDesktopLeftTocColumn ? 3 : 0'
            :xl='hasDesktopRightTocColumn ? 10 : false'
            :lg='hasDesktopRightTocColumn ? 9 : false'
            ...
            .page-edit-shortcuts(
              ...
              :class='hasDesktopRightTocColumn ? `is-right` : ``'
              )
              v-btn(
...
export default {
  ...
  computed: {
    ...
    isTaxonomyLayout () {
      return ['topic', 'category', 'taxonomy'].includes(this.pageLayoutClass)
    },
    heroLayoutClass () {
    ...
    hasTocContent () {
      return this.tocDecoded.length > 0
    },
    tocAllowedForPageLayout () {
      return this.tocEnabled || this.isTaxonomyLayout
    },
    hasTocColumn () {
      return this.tocPosition !== 'off' && this.hasTocContent && this.tocAllowedForPageLayout
    },
    hasDesktopTocColumn () {
      return this.hasTocColumn && this.$vuetify.breakpoint.lgAndUp
    },
    hasDesktopLeftTocColumn () {
      return this.hasDesktopTocColumn && this.tocPosition === 'left'
    },
    hasDesktopRightTocColumn () {
      return this.hasDesktopTocColumn && this.tocPosition === 'right'
    },
    contentLgColumns () {
  ...
  methods: {
    ...
    setupMobileInfoboxToggle () {
      ...
      infoboxes.forEach(infobox => {
        ...
        let headerTitle = ''
        const existingTitle = header.querySelector('.le-mobile-infobox-title')
        if (existingTitle) {
          headerTitle = existingTitle.textContent.trim()
        } else if (header.dataset.mobileInfoboxTitle) {
          headerTitle = header.dataset.mobileInfoboxTitle
        } else {
        ...
        const summaryText = this.buildInfoboxFieldSummary(infobox)
        header.dataset.mobileInfoboxTitle = headerTitle
        header.innerHTML = ''

        const titleNode = document.createElement('span')
        titleNode.className = 'le-mobile-infobox-title'
        titleNode.textContent = headerTitle
        header.appendChild(titleNode)

        if (summaryText) {
          const summaryNode = document.createElement('span')
          summaryNode.className = 'le-mobile-infobox-summary'
    ...
    buildInfoboxFieldSummary (infobox) {
      const rows = Array.from(infobox.querySelectorAll('.infobox-table tr'))
      const labels = rows.map(row => {
        const labelCell = row.querySelector('.infobox-label') || row.querySelector('td:first-child') || row.querySelector('th:first-child')
      ...
      return uniqueLabels.slice(0, 4).join(' • ')
    },
