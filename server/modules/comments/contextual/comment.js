...
module.exports = {
   ...
   */
  async init (config) {
    WIKI.logger.info('(COMMENTS/CONTEXTUAL) Initializing...')
    if (WIKI.data.commentProvider.config.akismet && WIKI.data.commentProvider.config.akismet.length > 2) {
      ...
      try {
        ...
        if (!isValid) {
          akismetClient = null
          WIKI.logger.warn('(COMMENTS/CONTEXTUAL) Akismet Key is invalid! [ DISABLED ]')
        } else {
          WIKI.logger.info('(COMMENTS/CONTEXTUAL) Akismet key is valid. [ OK ]')
        }
      } catch (err) {
        akismetClient = null
        WIKI.logger.warn('(COMMENTS/CONTEXTUAL) Unable to verify Akismet Key: ' + err.message)
      }
    } else {
      akismetClient = null
    }
    WIKI.logger.info('(COMMENTS/CONTEXTUAL) Initialization completed.')
  },
  /**
   * Create New Comment
   */
